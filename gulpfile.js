...
gulp.task('watchToStratTask', function () {
  gulp.watch([src + '/js/**.js'], ['browserReload','copyImgFolder']) // 當檔案有動靜，Reload
  gulp.watch([src + '/**.html'], ['browserReload','copyImgFolder']) // 當檔案有動靜，Reload
  gulp.watch([src + '/css/**.css'], ['browserReload','copyImgFolder']) // 當檔案有動靜，Reload

  // gulp.watch([webroot + '/**'], ['browserReload'])
})

gulp.task('default', ['copyImgFolder'], function () {
  if (isWatching) {
